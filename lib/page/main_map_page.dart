...
class MapState extends StatefulWidget {
  ...
  const MapState({Key? key, this.currentUser, required this.signout})
      : super(key: key);
...
class _MapState extends State<MapState> {
  ...
  processDirectionRequest() async {
    ...
    await _createPolylines(_currentPosition!.latitude,
        _currentPosition!.longitude, currentPoi.latitude, currentPoi.longitude);
  ...
  void processVisitLaterRequest(int locnid) async {
    ...
    http.Response resp = await http.post(
        Uri.parse("http://54.184.164.77:5000/adduserloc"),
  ...
  void addLike(int locnid, int likes) async {
    ...
    var body = json.encode(data);
    http.Response resp = await http.post(
        Uri.parse("http://54.184.164.77:5000/addlike"),
        headers: {"Content-Type": "application/json"},
        body: body);
    if (resp.statusCode == 200) {
  ...
  void addRating(int locnid, double existintRating, double newRating) async {
    ...
    var body = json.encode(data);
    http.Response resp = await http.post(
        Uri.parse("http://54.184.164.77:5000/addrating"),
        headers: {"Content-Type": "application/json"},
        body: body);
    if (resp.statusCode == 200) {
      print(resp.statusCode);
  ...
  void showMyPlaces() {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => VisitLaterLayout(email: email)),
    );
  }
      ...
      return MaterialApp(
        ...
        body: Stack(children: <Widget>[
          SlidingUpPanel(
            ...
            panel: infopanel.InfoPanelLayout(
                currentPoi,
